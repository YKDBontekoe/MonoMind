...
namespace Autonocraft.UI
{
    public class PauseMenuScreen
    {
        ...
        private readonly float[] _buttonHoverT = new float[4];
        ...
        private void UpdateMainPanel(Viewport viewport, KeyboardState kb, MouseState mouse, KeyboardState prevKb, MouseState prevMouse)
        {
            ...
            float resumeY = layout.CenterY - layout.S(36f);
            float settingsY = resumeY + buttonH + buttonSpacing;
            float mainMenuY = settingsY + buttonH + buttonSpacing;
            ...
            else if (settingsRect.Contains(mouse.X, mouse.Y)) _hoveredButton = 1;
            else if (mainMenuRect.Contains(mouse.X, mouse.Y)) _hoveredButton = 2;
            ...
            if (click)
            {
                ...
                else if (_hoveredButton == 1) _panelMode = PanelMode.Settings;
                else if (_hoveredButton == 2) MainMenuRequested = true;
                else if (_hoveredButton == 3) QuitRequested = true;
            }
        ...
        private void UpdateSettingsPanel(Viewport viewport, KeyboardState kb, MouseState mouse, KeyboardState prevKb, MouseState prevMouse)
        {
            ...
            float sliderY = layout.CenterY - layout.S(4f);
            float backY = sliderY + thumbSize + layout.S(48f);
        ...
        private void DrawMainPanel(UiLayout layout, float cx, float buttonW, float buttonH, float buttonSpacing, float alpha, float offsetY)
        {
            ...
            float resumeY = layout.CenterY - layout.S(36f) + offsetY;
            float settingsY = resumeY + buttonH + buttonSpacing;
            float mainMenuY = settingsY + buttonH + buttonSpacing;
            float quitY = mainMenuY + buttonH + buttonSpacing;
            ...
            DrawButton(cx, settingsY, buttonW, buttonH, "SETTINGS", 1, layout.S(1.4f), alpha);
            DrawButton(cx, mainMenuY, buttonW, buttonH, "MAIN MENU", 2, layout.S(1.4f), alpha);
            DrawButton(cx, quitY, buttonW, buttonH, "QUIT", 3, layout.S(1.5f), alpha);

            _ui.DrawCenteredText("WORLD SAVED ON EXIT", layout.Height - layout.S(48f) + offsetY, layout.S(1.0f), new Color(0.45f, 0.5f, 0.58f), 0.9f * alpha);
        ...
        private void DrawSettingsPanel(UiLayout layout, float cx, float buttonW, float buttonH, float alpha, float offsetY)
        {
            ...
            float thumbSize = layout.S(SliderThumbSize);
            float sliderX = cx - sliderW / 2f;
            float sliderY = layout.CenterY - layout.S(4f) + offsetY;
            float backY = sliderY + thumbSize + layout.S(48f);
