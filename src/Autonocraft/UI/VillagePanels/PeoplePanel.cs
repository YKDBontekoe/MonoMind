    ...
    public sealed class PeoplePanel : IVillagePanel
    {
        ...
        public void Draw(VillagePanelContext context)
        {
            ...
            foreach (var villager in EnumerateCitizens(village, context.Villagers))
            {
                ...
                bool hovered = context.HoveredButton == VillageScreen.PeopleVillagerButtonBase + villagerId;
            ...
            Color accent)
        {
            ...
            var detailLayout = PeopleDetailLayout.Compute(layout, villager, village, context.AssignFeedback);
            float talkY = detailLayout.TalkButtonY;
            ...
            DrawStyledButton(ui, x + pad, y + talkY, layout.S(96f), layout.S(ButtonHeight), "Talk",
                context.HoveredButton == VillageScreen.PeopleTalkButton,
                UiButtonStyle.Secondary, layout, alpha, !talkEnabled);
            ui.DrawString(talkHint, x + pad + layout.S(106f), y + talkY + layout.S(8f), layout.S(UiTheme.FontSmall),
                UiTheme.Hint, alpha);

            float feedbackY = y + talkY + layout.S(48f);
            if (!string.IsNullOrEmpty(context.AssignFeedback))
            {
                Color feedbackColor = context.AssignSuccess ? UiTheme.Success : UiTheme.Danger;
                ui.DrawString(context.AssignFeedback, x + pad, feedbackY, layout.S(UiTheme.FontSmall), feedbackColor, alpha);
            }

            float jobSectionY = detailLayout.JobSectionY;
            ui.DrawString("Assign job", x + pad, jobSectionY - layout.S(24f), layout.S(UiTheme.FontSection), UiTheme.Section, alpha, semiBold: true);
            ...
            for (int i = 0; i < AssignableJobs.Length; i++)
            {
                ...
                float jobY = jobSectionY + row * (jobH + jobGap);
                bool isCurrentJob = villager.CurrentJob == AssignableJobs[i].Job;
                bool hovered = context.HoveredButton == VillageScreen.PeopleJobButtonBase + i;
                var style = isCurrentJob ? UiButtonStyle.Primary : UiButtonStyle.Secondary;
                DrawStyledButton(ui, jobX, jobY, jobW, jobH, AssignableJobs[i].Label, hovered,
                    style, layout, alpha);
            }
        }
